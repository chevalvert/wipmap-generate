...
function perlinMap (octaves, opts) {
  ...
  return new Perlin2(random())
    .transform(function (x, y) {
      const offX = (x / CONFIG.width) + perlinOffset.x
      const offY = (y / CONFIG.height) + perlinOffset.y
...
module.exports = {
  ...
  randomFloat,
  // FIXME: correct offset from map to map
  perlin: (x, y) => perlin.gen((x / CONFIG.width) + perlinOffset.x, (y / CONFIG.height) + perlinOffset.y),
