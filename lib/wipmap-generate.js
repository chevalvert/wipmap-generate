...
module.exports = function generate (seedX, seedY, opts) {
  ...
  const landmarks = Object.entries(opts.landmarks)
  .reduce((instances, [name, landmark]) => {
    [].concat(landmark.length).forEach((instanceLength, instanceIndex) => {
      instances[`${name}-${instanceIndex}`] = Array(instanceLength).fill().map((_, index) => {
